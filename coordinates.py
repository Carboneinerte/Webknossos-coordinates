...
parser.add_argument('-k', '--keyword', action='store', default='Syn',
                    help='Keyword to look in node comments. !Case-insensitive!')
...
try:
    ...
    if not args.file_path:
        parser.error('Please provide the path to the NML file using -f or --file_path.')
    ...
    for i in range(len(file_content)):
        if com_looked.lower() in file_content[i].lower() :
